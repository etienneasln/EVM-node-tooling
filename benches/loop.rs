...
fn criterion_insert_then_clear(c:&mut Criterion){
    ...
    blueprint.insert(&mut connection);

    c.bench_function("Insert then clear", |b| b.iter(|| run_insert_then_clear(&mut connection,id+1,&payload,timestamp)));

    Blueprint::clear_after(&mut connection, id-1);

}

fn run_insert_then_clear(connection:&mut SqliteConnection,id:i32,payload:&Vec<u8>,timestamp:i32){
    let blueprint=Blueprint{
        id,payload:payload.clone(),timestamp
    };
