...
fn run_insert_blueprint_rusqlite(connection:&Connection,id:&mut i32,payload:&Vec<u8>,timestamp:i32){
    connection.execute("INSERT INTO blueprints (id,payload,timestamp) VALUES (?1,?2,?3)",(*id,payload,timestamp)).expect("Error");
...
fn criterion_apply_blueprint_18987875(c:&mut Criterion){
    ...
    c.bench_function("apply_blueprint_18987875", |b| b.iter(|| run_insert_blueprint_diesel(&mut connection,&mut insertindex,&payload,timestamp)));
...
fn criterion_apply_blueprint_18989033(c:&mut Criterion){
    ...
    c.bench_function("apply_blueprint_18989033", |b| b.iter(|| run_insert_blueprint_diesel(&mut connection,&mut insertindex,&payload,timestamp)));
...
fn criterion_apply_blueprint_18989013(c:&mut Criterion){
    ...
    c.bench_function("apply_blueprint_18989013", |b| b.iter(|| run_insert_blueprint_diesel(&mut connection,&mut insertindex,&payload,timestamp)));
...
fn criterion_insert_then_clear_in_memory(c:&mut Criterion){
    let mut group=c.benchmark_group("Insert then clear in memory");
    let rusqliteconnection=Connection::open_in_memory().expect(&format!("Could not open {}","in memory file"));
    ...
    let mut connection=establish_connection();
    let _ = diesel::sql_query("CREATE TABLE blueprints (
        id SERIAL PRIMARY KEY,
        payload BLOB NOT NULL,
        timestamp DATETIME NOT NULL
    ...
    // let _ = diesel::sql_query("PRAGMA synchronous = FULL;").execute(&mut connection);
    // let _= diesel::sql_query("PRAGMA journal_mode = WAL;").execute(&mut connection);

    group.bench_function("Insert then clear diesel", |b| b.iter(|| run_insert_then_clear_diesel(&mut connection,id,&payload,timestamp)));

    group.bench_function("Insert then clear rusqlite",|b| b.iter(|| run_insert_then_clear_rusqlite(&rusqliteconnection,id2,&payload,timestamp)));


}

fn criterion_insert_in_memory(c:&mut Criterion){
    let mut group=c.benchmark_group("Insert in memory");
    let rusqliteconnection=Connection::open_in_memory().expect(&format!("Could not open {}","in memory db"));
    let _ = rusqliteconnection.execute(
        "CREATE TABLE blueprints (
        id SERIAL PRIMARY KEY,
        payload BLOB NOT NULL,
    ...
    let _=Blueprint::clear_after(&mut connection, id-1);
    let _ = rusqliteconnection.execute("DELETE FROM blueprints WHERE id > ?1",params![id2-1]);

    

    group.bench_function("Insert diesel", |b| b.iter(|| run_insert_blueprint_diesel(&mut connection,&mut id,&payload,timestamp)));

    group.bench_function("Insert rusqlite",|b| b.iter(|| run_insert_blueprint_rusqlite(&rusqliteconnection,&mut id2,&payload,timestamp)));

    Blueprint::clear_after(&mut connection, id-1);
    let _ = rusqliteconnection.execute("DELETE FROM blueprints WHERE id > ?1",params![id2-1]);
...
fn criterion_insert(c:&mut Criterion){
    ...
    let rusqliteconnection=Connection::open("newtest.sqlite")
    .expect(&format!("Could not open {}","newtest.sqlite"));
    ...
    let _=Blueprint::clear_after(&mut connection, id-1);
    let _=Blueprint::clear_after(&mut connection, 0);

    let _ = diesel::sql_query("PRAGMA synchronous = FULL;").execute(&mut connection);
    // let _ = diesel::sql_query("PRAGMA synchronous = NORMAL;").execute(&mut connection);
    let _= diesel::sql_query("PRAGMA journal_mode = WAL;").execute(&mut connection);
...
fn criterion_insert_then_clear(c:&mut Criterion){
    let mut group=c.benchmark_group("Insert then clear");
    let rusqliteconnection=Connection::open("newtest.sqlite")
    .expect(&format!("Could not open {}","newtest.sqlite"));
    ...
    group.bench_function("Insert diesel", |b| b.iter(|| run_insert_then_clear_diesel(&mut connection,id,&payload,timestamp)));

    group.bench_function("Insert rusqlite",|b| b.iter(|| run_insert_then_clear_rusqlite(&rusqliteconnection,id2,&payload,timestamp)));
...
fn run_insert_then_clear_rusqlite(connection:&Connection,id:i32,payload:&Vec<u8>,timestamp:i32){
    connection.execute("INSERT INTO blueprints (id,payload,timestamp) VALUES (?1,?2,?3)",(id,payload,timestamp)).expect("Error");
    let _ = connection.execute("DELETE FROM blueprints WHERE id > ?1",params![id-1]);
}

criterion_group!(benches, 
    // criterion_insert_in_memory,
    // criterion_insert_then_clear_in_memory,
    criterion_insert,
