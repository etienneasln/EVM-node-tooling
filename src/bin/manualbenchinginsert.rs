...
use evmnodetooling::dieselsqlite::{establish_connection, rusqlite_connection, models::{Blueprint}};
...
fn main(){
    ...
    let rusqliteconn=rusqlite_connection();
    let start2=Instant::now();
    rusqliteconn.execute("INSERT INTO blueprints (id,payload,timestamp) VALUES (?1,?2,?3)",(id,payload,timestamp)).expect("Error");
